package com.ankita.waptag2018.visitor;
...
public class VisitorsActivity extends BaseAppCompatActivity {
    ...
    private class InsertVisitors extends AsyncTask<String,Void,String> {
        ...
        protected String doInBackground(String... strings) {
            ...
            JSONObject joVisitor=new JSONObject();
            try {

                joVisitor.put("name",edtName.getText().toString());
                joVisitor.put("compName", edtCompany.getText().toString());
                joVisitor.put("mobile", edtNo.getText().toString());
                joVisitor.put("email", edtMail.getText().toString());
                joVisitor.put("city", edtCity.getText().toString());
                joVisitor.put("state", STATE);
                joVisitor.put("country", COUNTRY);
                joVisitor.put("businessCat", BUSINESS_CATEGORY);
                Postdata postdata=new Postdata();
                String pdVisitor=postdata.post(WEB_KEY + API_VISITOR_REGISTRATION,joVisitor.toString());
                JSONObject j=new JSONObject(pdVisitor);
